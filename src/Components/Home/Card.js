...
function Card(props) {
    ...
    const filteredRooms =  card_list.filter(data => (info.beds === "" || info.beds === data.beds) && (info.baths === "" || info.baths === data.baths) && (info.type === "" || info.type === data.type) &&  ( info.location === "" || info.location === data.city) && (info.status === "" || info.status.replace(/\s/g, "") === data.forSale))
